...
import static gestor.Visao.TelaTransferenciaValoresInternos.jSaldoContaAtiva;
import static gestor.Visao.TelaTransferenciaValoresInternos.jFotoInterno;
import static gestor.Visao.TelaTransferenciaValoresInternos.jIdInternoCrc;
import static gestor.Visao.TelaTransferenciaValoresInternos.jNomeInternoCrc;
import static gestor.Visao.TelaTransferenciaValoresInternos.jSituacao;
 ...
 * @author Ronaldo
 */
public class TelaPesqInternoTransferenciaValoresInativos extends javax.swing.JInternalFrame {
    ...
    String tipo = "DI"; // SE O CAMPO ESTIVER VAZIO, MOSTRA OS DADOS, CASO CONTRÁRIO NÃO MOSTRA.
    public static String codDeposito = "";
    String registro = "";

    /**
     * Creates new form TelaPesquisaEntradaInternos
     */
    public TelaPesqInternoTransferenciaValoresInativos() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        jTabelaInterno.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Código", "Nome do Interno", "Registro", "Valor R$", "Data Cadastro"
            }
        ));
    ...
    private void jBtNomeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtNomeActionPerformed
        ...
        if (jPesqNome.getText().equals("")) {
            ...
            preencherTabelaNome("SELECT * FROM PRONTUARIOSCRC "
                    ...
                    + "INNER JOIN DEPOSITO_INATIVOS "
                    + "ON PRONTUARIOSCRC.IdInternoCrc=DEPOSITO_INATIVOS.IdInternoCrc "
                    + "WHERE NomeInternoCrc LIKE'%" + jPesqNome.getText() + "%' "
                    + "AND SituacaoCrc='" + situacaoEntrada + "' "
                    + "AND Tipo='" + tipo + "' "
                    + "OR NomeInternoCrc LIKE'%" + jPesqNome.getText() + "%' "
                    + "AND SituacaoCrc='" + situacaoRetorno + "' "
                    + "AND Tipo='" + tipo + "'");
        }
    ...
    private void jTabelaInternoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTabelaInternoMouseClicked
        ...
        flag = 1;
        if (flag == 1) {
            registro = "" + jTabelaInterno.getValueAt(jTabelaInterno.getSelectedRow(), 2);
        ...
        if (jPesqNome.getText().isEmpty()) {
            ...
            try {
                conecta.executaSQL("SELECT * FROM PRONTUARIOSCRC "
                        ...
                        + "AND IdDepIna='" + registro + "'");
                conecta.rs.first();
                codDeposito = conecta.rs.getString("IdDepIna");
                ...
                saldoTotalCredito = conecta.rs.getDouble("ValorDeposito");
                DecimalFormat sd = new DecimalFormat("#,##0.00");
                String vlLiquido = sd.format(saldoTotalCredito);
                jSaldoContaAtiva.setText(String.valueOf(vlLiquido));
                //
    ...
    private void jCheckBox1ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jCheckBox1ItemStateChanged
        ...
        if (evt.getStateChange() == evt.SELECTED) {
            this.preencherTodosInternos("SELECT * FROM PRONTUARIOSCRC "
                    ...
                    + "INNER JOIN DEPOSITO_INATIVOS "
                    + "ON PRONTUARIOSCRC.IdInternoCrc=DEPOSITO_INATIVOS.IdInternoCrc "
                    + "WHERE SituacaoCrc='" + situacaoEntrada + "' "
                    + "AND Tipo='" + tipo + "' "
                    + "OR SituacaoCrc='" + situacaoRetorno + "'"
                    + "AND SituacaoCrc='" + situacaoEntrada + "' "
                    + "AND Tipo='" + tipo + "'");
        } else {
            limparTabela();
  ...
  // Método de pesquisa pela Descrição
    ...
    public void preencherTabelaNome(String sql) {
        ...
        String[] Colunas = new String[]{"Código", "Nome do Interno", "Registro", "Valor Depósito", "Data Depósito"};
        conecta.abrirConexao();
        conecta.executaSQL(sql);
        try {
            conecta.rs.first();
            do {
                valorSaldo = conecta.rs.getDouble("ValorDeposito");
                ...
                dados.add(new Object[]{conecta.rs.getInt("IdInternoCrc"), conecta.rs.getString("NomeInternoCrc"), conecta.rs.getString("IdDepIna"), vlSaldo, dataCadastro});
    ...
    public void preencherTodosInternos(String sql) {
        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"Código", "Nome do Interno", "Registro", "Valor Depósito", "Data Cadastro"};
        conecta.abrirConexao();
        try {
            conecta.executaSQL(sql);
            conecta.rs.first();
    ...
    public void buscarInternosMatricula(String sql) {
        ArrayList dados = new ArrayList();
        String[] Colunas = new String[]{"CódigoD", "Nome do Interno", "Registro", "Valor Depósito", "Data Deposito"};
        conecta.abrirConexao();
        conecta.executaSQL(sql);
        try {
            conecta.rs.first();
    ...
    public void buscarCredito() {
        ...
        try {
            conecta.executaSQL("SELECT * FROM SALDO_VALORES_INATIVOS "
                    + "WHERE IdInternoCrc='" + jTabelaInterno.getValueAt(jTabelaInterno.getSelectedRow(), 0) + "'");
            conecta.rs.last();
            do {
                saldoTotalCredito = conecta.rs.getDouble("SaldoAtual");
            } while (conecta.rs.next());
        } catch (Exception e) {
        }
    }
