...
import static gestor.Visao.TelaRequisicaoMateriaisInternosAC.jDescricaoProduto;
import static gestor.Visao.TelaRequisicaoMateriaisInternosAC.jCodProduto;
import static gestor.Visao.TelaRequisicaoMateriaisInternosAC.jCodigoBarras;
...
import static gestor.Visao.TelaRequisicaoMateriaisInternosAC.jComboBoxUnidade;
 ...
 * @author user
 */
public class TelaPesqProdutoRequisicaoSEAC extends javax.swing.JInternalFrame {
    ...
    float valorUnitario;
    String utilizacaoRequisicao = "Não";
    String modulo = "A";

    /**
     * Creates new form TelaPesqProdutoFarmacia
     */
    public TelaPesqProdutoRequisicaoSEAC() {
        initComponents();
    }
    ...
    private void jBtPesqDescricaoProdActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtPesqDescricaoProdActionPerformed
        ...
        if (jPesqDescricaoProdutos.getText().equals("")) {
            ...
            preencherTabelaProdutos("SELECT * FROM ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC "
                    + "INNER JOIN PRODUTOS_AC "
                    + "ON ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC.IdProd=PRODUTOS_AC.IdProd "
                    + "INNER JOIN LOCAL_ARMAZENAMENTO_AC "
                    + "ON PRODUTOS_AC.IdLocal=LOCAL_ARMAZENAMENTO_AC.IdLocal INNER "
                    + "JOIN LOTE_PRODUTOS_AC "
                    + "ON ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC.IdProd=LOTE_PRODUTOS_AC.IdProd "
                    + "WHERE PRODUTOS_AC.DescricaoProd LIKE'%" + jPesqDescricaoProdutos.getText() + "%' "
                    + "AND PRODUTOS_AC.StatusProd='" + statusProd + "' "
                    + "AND ReqAtend='" + utilizacaoRequisicao + "'");
        }
    ...
    private void jCheckBoxPesqTodosProdItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jCheckBoxPesqTodosProdItemStateChanged
        ...
        if (evt.getStateChange() == evt.SELECTED) {
            this.preencherTabelaProdutos("SELECT * FROM ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC "
                    + "INNER JOIN PRODUTOS_AC "
                    + "ON ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC.IdProd=PRODUTOS_AC.IdProd "
                    + "INNER JOIN LOCAL_ARMAZENAMENTO_AC "
                    + "ON PRODUTOS_AC.IdLocal=LOCAL_ARMAZENAMENTO_AC.IdLocal INNER "
                    + "JOIN LOTE_PRODUTOS_AC "
                    + "ON ITENS_REQUISICAO_PRODUTOS_INTERNOS_SEAC.IdProd=LOTE_PRODUTOS_AC.IdProd "
                    + "WHERE PRODUTOS_AC.StatusProd='" + statusProd + "'AND ReqAtend='" + utilizacaoRequisicao + "'");
        } else {
            limparTabela();
        }
