...
public class ControleCursosDiversos {
    ...
    public CursosDiversos incluirCursos(CursosDiversos objCursos) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO CURSOS (StatusCurso, DataCurso,DescricaoCurso,usuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?)");
    ...
    public CursosDiversos alterarCursos(CursosDiversos objCursos) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE CURSOS SET StatusCurso=?, DataCurso=?,DescricaoCurso=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdCurso='" + objCursos.getIdCurso() + "'");
            pst.setString(1, objCursos.getStatusCurso());
            pst.setTimestamp(2, new java.sql.Timestamp(objCursos.getDataCurso().getTime()));
            pst.setString(3, objCursos.getDescricaoCurso());
