 ...
 * @author ronaldo
 */
public class ControleHistoricoCompraProdutoNUTRI {
    ...
    public HistoricoCompraAcervo incluirHistoricoComprasProdutoNUTRI(HistoricoCompraAcervo objHistCompra) {
        buscarProdutosAC(objHistCompra.getNomeLivro());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_COMPRA_NUTRI (NfCompra,DataMov,IdNfEntrada,IdProd,QtdeCompra,ValorUnit,ValorTotal) VALUES(?,?,?,?,?,?,?)");
    ...
    public HistoricoCompraAcervo alterarHistoricoComprasProdutoNUTRI(HistoricoCompraAcervo objHistCompra) {
        buscarProdutosAC(objHistCompra.getNomeLivro());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE HISTORICO_COMPRA_NUTRI SET NfCompra=?,DataMov=?,IdNfEntrada=?,IdProd=?,QtdeCompra=?,ValorUnit=?,ValorTotal=? WHERE IdNfEntrada='" + objHistCompra.getIdCompra() + "'AND IdProd='" + objHistCompra.getIdLivro() + "'");
            pst.setInt(1, objHistCompra.getNfCompra());
            pst.setTimestamp(2, new java.sql.Timestamp(objHistCompra.getDataMov().getTime()));
            pst.setInt(3, objHistCompra.getIdCompra());
            pst.setInt(4, codProduto);
    ...
    public HistoricoCompraAcervo excluirHistoricoComprasProdutoNUTRI(HistoricoCompraAcervo objHistCompra) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM HISTORICO_COMPRA_NUTRI WHERE IdNfEntrada='" + objHistCompra.getIdCompra() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
