 ...
 * @author Ronaldo
 */
public class ControleNFeCompras {
    ...
    public NotaFiscalCompra incluirNFeCompras(NotaFiscalCompra objNFEComprasv) {
        buscarFornecedor(objNFEComprasv.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO NF_COMPRAS "
    ...
    public NotaFiscalCompra alterarNFeCompras(NotaFiscalCompra objNFEComprasv) {
        buscarFornecedor(objNFEComprasv.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE NF_COMPRAS SET "
                    + "Modelo=?,SerieNf=?,NumeroNf=?,StatusNf=?,IdLocal=?,IdForn=?,DataEmissao=?,Tipodocumento=?,FormaPagamento=?, "
                    + "DataEntrada=?,BaseCalculoICMS=?,ValorICMS=?,BaseCalculoICMSSub=?,ValorICMSSub=?,ValorTotalProdutos=?, "
                    + "ValorTotalFrete=?,ValorTotalSeguro=?,ValorTotalDesconto=?,ValorTotalIPI=?,ValorTotalNFE=?,ObservacaoNF=?, "
    ...
    public NotaFiscalCompra excluirNFeCompras(NotaFiscalCompra objNFEComprasv) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM NF_COMPRAS WHERE IdNfEntrada='" + objNFEComprasv.getIdNfEntrada() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
     ...
     public NotaFiscalCompra finalizarNFeCompras(NotaFiscalCompra objNFEComprasv) {
       
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE NF_COMPRAS SET StatusNf=? WHERE IdNfEntrada='" + objNFEComprasv.getIdNfEntrada() + "'");           
