...
import static gestor.Visao.TelaPRORES.pSTATUS_ANDAMENTO;

/**
 *
 * @author Socializa TI 02
 */
public class ControleListaIndicadoresAcompanhamentoPSICOLOGIA {
    ...
    public List<IndicadoresAcompanhamento> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT "
                    ...
                    + "AND STATUS_INTERNO_PSICOLOGIA.StatusAtendimento='" + pTRATAMENTO + "' "
            ...
            while (conecta.rs.next()) {
                ...
                if (pDigiProd.getTratamento().equals("Em Andamento")) {
                    pSTATUS_ANDAMENTO++;
                    pDigiProd.setQtdTratamento(pSTATUS_ANDAMENTO);
                }                
                listaInternosPavilhaoSelecionados.add(pDigiProd);
            }
            return listaInternosPavilhaoSelecionados;
        } catch (SQLException ex) {
            Logger.getLogger(ControleListaIndicadoresAcompanhamentoPSICOLOGIA.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
