 ...
 * @author ronaldo
 */
public class ControleParcelasComprasFAR {
    ...
    public ParcelasPagtoCompras incluirParcelasComprasFAR(ParcelasPagtoCompras objParCompras) {
        buscarFornecedor(objParCompras.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO PARCELAS_COMPRAS_FAR "
    ...
    public ParcelasPagtoCompras alterarParcelasComprasFAR(ParcelasPagtoCompras objParCompras) {
        buscarFornecedor(objParCompras.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE PARCELAS_COMPRAS_FAR SET "
                    + "IdNfEntrada=?,IdForn=?,DataVcto=?,ValorParcela=?,ValorTotalParcelas=?, "
    ...
    public ParcelasPagtoCompras excluirParcelasComprasFAR(ParcelasPagtoCompras objParCompras) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM PARCELAS_COMPRAS_FAR WHERE IdParc='" + objParCompras.getIdParc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
