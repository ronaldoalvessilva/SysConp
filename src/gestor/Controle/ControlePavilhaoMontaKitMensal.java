 ...
 * @author ronal
 */
public class ControlePavilhaoMontaKitMensal {
    ...
    public List<PavilhaoInternoMontaKit> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT DISTINCT KITS_MENSAL_INTERNOS.IdInternoCrc, "
                    + "PRONTUARIOSCRC.Cnc,PRONTUARIOSCRC.NomeInternoCrc, "
                    + "KITS_MENSAL_INTERNOS.KitPago,KITS_MENSAL_INTERNOS.Utilizado, "
                    + "KITS_MENSAL_INTERNOS.DataPrevisaoPro,PAVILHAO.IdPav,PAVILHAO.DescricaoPav "
                    + "FROM PRONTUARIOSCRC "
            ...
            Logger.getLogger(ControlePavilhaoMontaKitMensal.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
