...
import static gestor.Visao.TelaAberturaRegistroSistema.pPERFIL_carcerario;
 ...
 */
public class ListagemPerfilCarcerarioSSM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        pPERFIL_carcerario = 0;
        List<FechamentoRegistros> listaTodasNovaEntrada = new ArrayList<FechamentoRegistros>();
        conecta.abrirConexao();
        try {
            conecta.executaSQL("SELECT StatusPerfil "
                    + "FROM PERFIL_CARCERARIO_INTERNO "
                    + "WHERE StatusPerfil='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataPerfil BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusPerfil"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pPERFIL_carcerario = pPERFIL_carcerario + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemPerfilCarcerarioSSM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
