...
import static gestor.Visao.TelaAtividadesMensalUnidade.pQUANTIDADE_EVO_FAMILIA;
 ...
 * @author ronal
 */
public class ListagemEvolucaoADMServicoSocialFamilia {

    //MODELO DA LISTAGEM PARA EVOLUÇÃO DO ATENDIMENTO A FAMÍLIA DO SERVIÇO SOCIAL. AINDA NÃO FOI DEVIDAMENTE IMPLEMENTADA.
    ...
    public List<AtividadesMensalRealizadaUnidades> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT DataEvolVisita "
                    + "FROM EVOLUCAO_ATENDIMENTO_FAMILIA "
                    + "WHERE CONVERT(DATE, DataEvolVisita) BETWEEN'" + pDATA_INICIAL + "' "
                    + "AND '" + pDATA_FINAL + "' ");                   
            while (conecta.rs.next()) {
                AtividadesMensalRealizadaUnidades pAtivaSSF = new AtividadesMensalRealizadaUnidades();
                pAtivaSSF.setDataAtendimento(conecta.rs.getDate("DataEvolVisita"));
                listaAtendInternos.add(pAtivaSSF);
                pQUANTIDADE_EVO_FAMILIA = pQUANTIDADE_EVO_FAMILIA + 1;
            }
