...
import static gestor.Visao.TelaAberturaRegistroSistema.pSAIDAS_CRC;
 ...
 */
public class ListagemRegistroSaidaCrcM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusSai "
                    + "FROM REGSAIDACRC "
                    + "WHERE StatusSai='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DatalancaMov BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusSai"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pSAIDAS_CRC = pSAIDAS_CRC + 1;
            }
