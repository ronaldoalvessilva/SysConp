 ...
 * @author Ronaldo
 */
public class ControleItensDoencaAdicional {
    ...
    public ItensDoencas incluirDoencas(ItensDoencas objItensDoenca) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_ADMISSAO_DOENCAS_ADICIONAL (DataLanc,IdAdmADI,IdDoenca) VALUES(?,?,?)");
    ...
    public ItensDoencas alterarDoencas(ItensDoencas objItensDoenca) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_ADMISSAO_DOENCAS_ADICIONAL SET DataLanc=?,IdLanc=?,IdDoenca=? WHERE IdAdmADI='" + objItensDoenca.getIdLanc() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objItensDoenca.getDataLanc().getTime()));
            pst.setInt(2, objItensDoenca.getIdLanc());
            pst.setInt(3, codDoenca);
            pst.executeUpdate();
    ...
    public ItensDoencas excluirDoencas(ItensDoencas objItensDoenca) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_ADMISSAO_DOENCAS_ADICIONAL WHERE IdAdmADI='" + objItensDoenca.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (Exception e) {
