 ...
 * @author ronaldo
 */
public class ControleNFeComprasFAR {
    ConexaoBancoDados conecta = new ConexaoBancoDados();
    NotaFiscalCompra objNFEComprasv = new NotaFiscalCompra();

    int codForn;

    public NotaFiscalCompra incluirNFeComprasFAR(NotaFiscalCompra objNFEComprasv) {
        buscarFornecedor(objNFEComprasv.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO NF_COMPRAS_FAR "
    ...
    public NotaFiscalCompra alterarNFeComprasFAR(NotaFiscalCompra objNFEComprasv) {
        buscarFornecedor(objNFEComprasv.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE NF_COMPRAS_FAR SET "
                    + "Modelo=?,SerieNf=?,NumeroNf=?,StatusNf=?,IdLocal=?,IdForn=?,DataEmissao=?,Tipodocumento=?,FormaPagamento=?, "
                    + "DataEntrada=?,BaseCalculoICMS=?,ValorICMS=?,BaseCalculoICMSSub=?,ValorICMSSub=?,ValorTotalProdutos=?, "
                    + "ValorTotalFrete=?,ValorTotalSeguro=?,ValorTotalDesconto=?,ValorTotalIPI=?,ValorTotalNFE=?,ObservacaoNF=?, "
    ...
    public NotaFiscalCompra excluirNFeComprasFAR(NotaFiscalCompra objNFEComprasv) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM NF_COMPRAS_FAR WHERE IdNfEntrada='" + objNFEComprasv.getIdNfEntrada() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
     ...
     public NotaFiscalCompra finalizarNFeComprasFAR(NotaFiscalCompra objNFEComprasv) {
       
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE NF_COMPRAS_FAR SET StatusNf=? WHERE IdNfEntrada='" + objNFEComprasv.getIdNfEntrada() + "'");           
