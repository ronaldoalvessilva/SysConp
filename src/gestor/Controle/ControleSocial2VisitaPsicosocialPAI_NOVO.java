 ...
 * @author ronaldo
 */
public class ControleSocial2VisitaPsicosocialPAI_NOVO {
    ...
    public Social2FamiliarPsicosocial incluirVisitaPsicoSocial(Social2FamiliarPsicosocial objSocial2Vista) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO FICHA_CADASTRO_PAI_CCGF_VF1 (IdPai,IdInternoCrc,IdVisita,Ocupacao,Idade,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public Social2FamiliarPsicosocial alterarVisitaPsicoSocial(Social2FamiliarPsicosocial objSocial2Vista) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE FICHA_CADASTRO_PAI_CCGF_VF1 SET IdPai=?,IdInternoCrc=?,IdVisita=?,Ocupacao=?,Idade=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdVF1='" + objSocial2Vista.getIdSol2Visita() + "'");
            pst.setInt(1, objSocial2Vista.getIdPai());
            pst.setInt(2, codInt);
            pst.setInt(3, codVisita);
            pst.setString(4, objSocial2Vista.getOcupacao());
    ...
    public Social2FamiliarPsicosocial excluirVisitaPsicoSocial(Social2FamiliarPsicosocial objSocial2Vista) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM FICHA_CADASTRO_PAI_CCGF_VF1 WHERE IdVF1='" + objSocial2Vista.getIdSol2Visita() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
