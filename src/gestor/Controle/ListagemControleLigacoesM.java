...
import static gestor.Visao.TelaAberturaRegistroSistema.pCONTROLE_ligacoes;
 ...
 */
public class ListagemControleLigacoesM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        pCONTROLE_ligacoes = 0;
        List<FechamentoRegistros> listaTodasNovaEntrada = new ArrayList<FechamentoRegistros>();
        conecta.abrirConexao();
        try {
            conecta.executaSQL("SELECT StatusControl "
                    + "FROM CONTROLIGA "
                    + "WHERE StatusControl='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataControl BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusControl"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pCONTROLE_ligacoes = pCONTROLE_ligacoes + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemControleLigacoesM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
