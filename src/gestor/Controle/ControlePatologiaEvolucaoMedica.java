 ...
 * @author Ronaldo
 */
public class ControlePatologiaEvolucaoMedica {
    ...
    public PatologiaEvolucaoPsiquiatrica incluirPatologiaPsiquiatria(PatologiaEvolucaoPsiquiatrica objPatEvol) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO PATOLOGIA_EVOLUCAO_MEDICA (DataReg,IdItem,IdInternoCrc,IdDoenca,TipoAna,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public PatologiaEvolucaoPsiquiatrica alterarPatologiaPsiquiatria(PatologiaEvolucaoPsiquiatrica objPatEvol) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE PATOLOGIA_EVOLUCAO_MEDICA SET DataReg=?,IdItem=?,IdInternoCrc=?,IdDoenca=?,TipoAna=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdPatMed='" + objPatEvol.getIdPatPsi() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objPatEvol.getDataReg().getTime()));
            pst.setInt(2, objPatEvol.getIdItem());
            pst.setInt(3, objPatEvol.getIdInternoCrc()); 
            pst.setInt(4, codDoenca);
    ...
    public PatologiaEvolucaoPsiquiatrica excluirPatologiaPsiquiatria(PatologiaEvolucaoPsiquiatrica objPatEvol) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM PATOLOGIA_EVOLUCAO_MEDICA WHERE IdPatMed='" + objPatEvol.getIdPatPsi() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
