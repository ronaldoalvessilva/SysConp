...
import static gestor.Visao.TelaAberturaRegistroSistema.pATUALIZA_documento;
 ...
 */
public class ListagemAtualizacaoDocumentosInternosM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        pATUALIZA_documento = 0;
        List<FechamentoRegistros> listaTodasNovaEntrada = new ArrayList<FechamentoRegistros>();
        conecta.abrirConexao();
        try {
            conecta.executaSQL("SELECT StatusDoc "
                    + "FROM ATUALIZACAO_DOCUMENTOS_INTERNOS "
                    + "WHERE StatusDoc='" + pSTATUS_NOVA_ENTRADA + "' "
            ...
            while (conecta.rs.next()) {
                ...
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusDoc"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pATUALIZA_documento = pATUALIZA_documento + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemAtualizacaoDocumentosInternosM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
