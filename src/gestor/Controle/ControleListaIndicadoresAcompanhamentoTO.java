...
import static gestor.Visao.TelaPRORES.pSTATUS_SITUACAO;
 ...
 * @author ronal
 */
public class ControleListaIndicadoresAcompanhamentoTO {
        ...
        try {
            ...
            while (conecta.rs.next()) {
                ...
                if (pDigiProd.getSituacaoAluno().equals("Aprovado")) {
                    pSTATUS_SITUACAO++;
                    pDigiProd.setQtdProfissional(pSTATUS_SITUACAO);
                }                
                listaInternosPavilhaoSelecionados.add(pDigiProd);
            }
            return listaInternosPavilhaoSelecionados;
        } catch (SQLException ex) {
            Logger.getLogger(ControleListaIndicadoresAcompanhamentoTO.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
