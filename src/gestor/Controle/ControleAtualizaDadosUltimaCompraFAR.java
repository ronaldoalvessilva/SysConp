 ...
 * @author ronaldo
 */
public class ControleAtualizaDadosUltimaCompraFAR {
    ...
    public ProdutoMedicamento alterarDadosComprasFAR(ProdutoMedicamento objProdMed) {
    ...
    public HistoricoProdutoFornecedor incluirDadosFornecedorFAR(HistoricoProdutoFornecedor objHistorForn) {
        buscarFornecedor(objHistorForn.getNomeFornecedor());
        buscarProduto(objHistorForn.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_PRODUTOS_FORNECEDOR_FAR (NfCompra,DataMov,IdForn,IdItem,IdProd,QtdeCompra,ValorUnit) VALUES(?,?,?,?,?,?,?)");
