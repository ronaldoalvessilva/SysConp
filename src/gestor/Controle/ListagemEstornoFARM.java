...
import static gestor.Visao.TelaFechamentoSistema.pESTORNO_far;
 ...
 */
public class ListagemEstornoFARM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusEst "
                    + "FROM ESTORNO_PRODUTOS_FAR "
                    + "WHERE StatusEst='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataEst BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusEst"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pESTORNO_far = pESTORNO_far + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemEstornoFARM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
