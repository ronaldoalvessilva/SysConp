 ...
 * @author Ronaldo
 */
public class ControleHistoricoEducacional {
    ...
    public HistoricoEducacionalLaboral incluirHistoricoEduca(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO TO_HISTORICO_EDUCACIONAL (IdLanc,IdInternoCrc,EscreveProprioNome,SabeLerEscrever,NivelInstrucao,InteresseEstudar,CursoProfissionalizante,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possível INSERIR os Dados. (TO_HISTORICO_EDUCACIONAL)\nERRO: " + e);
        }
        conecta.desconecta();
    ...
    public HistoricoEducacionalLaboral alterarHistoricoEduca(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE TO_HISTORICO_EDUCACIONAL SET IdLanc=?,IdInternoCrc=?,EscreveProprioNome=?,SabeLerEscrever=?,NivelInstrucao=?,InteresseEstudar=?,CursoProfissionalizante=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdHistoricoEdu='" + objHistEducLabor.getIdHistoricoEdu() + "'");
            pst.setInt(1, objHistEducLabor.getIdAtend());
            pst.setInt(2, codInterno);
            pst.setString(3, objHistEducLabor.getEscreveProprioNome());
            pst.setString(4, objHistEducLabor.getSabeLerEscrever());
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possível ALTERAR os Dados. (TO_HISTORICO_EDUCACIONAL)\nERRO: " + e);
        }
        conecta.desconecta();
        return objHistEducLabor;
    }
    ...
    public HistoricoEducacionalLaboral excluirHistoricoEduca(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM TO_HISTORICO_EDUCACIONAL WHERE IdHistoricoEdu='" + objHistEducLabor.getIdHistoricoEdu() + "'");
            pst.executeUpdate();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Não Foi possível EXCLUIR os Dados. (TO_HISTORICO_EDUCACIONAL)\nERRO: " + e);
        }
        conecta.desconecta();
        return objHistEducLabor;
    }
