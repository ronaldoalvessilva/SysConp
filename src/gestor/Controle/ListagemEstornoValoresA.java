...
import static gestor.Visao.TelaAberturaTotalSistema.pESTORNO_valores;
 ...
 */
public class ListagemEstornoValoresA {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusLanc "
                    + "FROM ESTORNO_DEPOSITO_SAQUE "
                    + "WHERE StatusEstorno='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataLanc<='" + pDATA_PESQUISA_FECHAMENTO + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusEstorno"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pESTORNO_valores = pESTORNO_valores + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemEstornoValoresA.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
