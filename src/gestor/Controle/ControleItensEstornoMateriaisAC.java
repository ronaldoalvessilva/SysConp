 ...
 * @author ronaldo
 */
public class ControleItensEstornoMateriaisAC {
    ...
    public ItensRequisicaoMateriaisInternos incluirRequisicaoMaterialInternos(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_ESTORNO_PRODUTOS_AC (IdEst,IdProd,CodigoBarras,UnidadeProd,QtdItem,ValorUnitarioItem,ValorTotalItem,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensRequisicaoMateriaisInternos alterarRequisicaoMaterialInternos(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_ESTORNO_PRODUTOS_AC SET IdEst=?,IdProd=?,CodigoBarras=?,UnidadeProd=?,QtdItem=?,ValorUnitarioItem=?,ValorTotalItem=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.setInt(1, objItensReqMatInter.getIdReq());
            pst.setInt(2, codProd);
            pst.setString(3, objItensReqMatInter.getCodigoBarras());
            pst.setString(4, objItensReqMatInter.getUnidadeProd());
    ...
    public ItensRequisicaoMateriaisInternos excluirRequisicaoMaterialInternos(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_ESTORNO_PRODUTOS_AC WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public ItensRequisicaoMateriaisInternos modificarRequisicaoMaterialAvulso(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_AVULSA_PRODUTOS  SET EstornoProduto=? WHERE IdProd='" + objItensReqMatInter.getIdProd() + "'AND IdReq='" + objItensReqMatInter.getIdReq() + "'");
    ...
    public ItensRequisicaoMateriaisInternos modificarRequisicaoMaterialInternos(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_PRODUTOS_INTERNOS  SET EstornoProduto=? WHERE IdProd='" + objItensReqMatInter.getIdProd() + "'AND IdReq='" + objItensReqMatInter.getIdReq() + "'");
            pst.setString(1, objItensReqMatInter.getEstornoProduto());
            pst.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Não Foi possivel ALTERAR itens da requisição.\nERRO: " + ex);
