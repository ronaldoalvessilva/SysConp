 ...
 * @author ronaldo
 */
public class ControleAdmissaoEnfermagem_AD {
    ...
    public AdmissaoEnfermagem incluirAdmissaoEnfermagem(AdmissaoEnfermagem objAdmEnfermagem) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ADMISSAO_ENFERMEIRA_COMPLEMENTAR (StatusLanc,DataLanc,IdInternoCrc,EstadoEmocional,SonoRepouso,NivelConsciencia,PressaoArterial,Hemograma,Temperatura,FrequenciaRespiratoria, "
                    ...
                    + "Escabiose,Dst,Hanseniase,Hpv,IdLanc) "
                    + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            pst.setInt(62, objAdmEnfermagem.getIdLanc());
            pst.execute();
    ...
    public AdmissaoEnfermagem alterarAdmissaoEnfermagem(AdmissaoEnfermagem objAdmEnfermagem) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ADMISSAO_ENFERMEIRA_COMPLEMENTAR SET StatusLanc=?,DataLanc=?,IdInternoCrc=?,EstadoEmocional=?,SonoRepouso=?,NivelConsciencia=?,PressaoArterial=?,Hemograma=?,Temperatura=?,FrequenciaRespiratoria=?, "
                    + "Peso=?,FrequenciaCardiaca=?,UsaMedicamentos=?,QualMedicacao=?,Locomocao=?,AcuidadeVisual=?,AcuidadeAuditiva=?,FuncaoMotora=?,QualFuncaoMotora=?,FalaLinguagem=?,"
                    + "QualFala=?,Pele=?,Mucosa=?,TipoPele=?,Localizacao=?,Cabelos=?,Boca=?,FuncaoRespiratoria=?,Torax=?,FuncaoIntestinal=?,DiasConstipado=?,Abdome=?,"
                    + "FuncaoVesical=?,Genitalia=?,QualGenitalia=?,Vacinado=?,QuaisVacinas=?,Vdrl=?,HepatiteC=?,HepatiteB=?,Hiv=?,Cirurgias=?,QuaisCirurgias=?,UsuarioDrogas=?,QuaisDrogas=?,"
                    + "PortadorDoenca=?,QuaisDoencas=?,Alergias=?,QuaisAlergias=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=?,Sifilis=?,Diabetes=?,Hipertensao=?,Tuberculose=?,Escabiose=?,Dst=?,Hanseniase=?,Hpv=?,IdLanc=? WHERE IdADME='" + objAdmEnfermagem.getIdADME()+ "'");
            pst.setString(1, objAdmEnfermagem.getStatusLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objAdmEnfermagem.getDataLanc().getTime()));
            pst.setInt(3, codInterno);
            pst.setInt(4, objAdmEnfermagem.getEstadoEmocional());
    ...
    public AdmissaoEnfermagem excluiAdmissaoEnfermagem(AdmissaoEnfermagem objAdmEnfermagem) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ADMISSAO_ENFERMEIRA_COMPLEMENTAR WHERE IdADME='" + objAdmEnfermagem.getIdADME() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public AdmissaoEnfermagem finalizarAdmissaoEnfermagem(AdmissaoEnfermagem objAdmEnfermagem) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ADMISSAO_ENFERMEIRA_COMPLEMENTAR SET StatusLanc=? WHERE IdADME='" + objAdmEnfermagem.getIdADME() + "'");
            pst.setString(1, objAdmEnfermagem.getStatusLanc());
            pst.executeUpdate();
        } catch (SQLException ex) {
