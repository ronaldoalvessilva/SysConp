 ...
 * @author ronaldo
 */
public class ControleAtualizaSaldoInventario {
    ...
    public ProdutoMedicamento alterarEstoqueProduto(ProdutoMedicamento objProdMed) {
        ...
        try {
          ...
          //  pst.setString(1, objProdMed.getLote());
    ...
    public ProdutoMedicamento atualizarEstoqueProduto(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE SALDO_ESTOQUE_AC SET SaldoAtual=? WHERE IdProd='" + objProdMed.getIdProd() + "'");                      
            pst.setFloat(1, objProdMed.getSaldoAtual());
