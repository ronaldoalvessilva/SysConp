 ...
 * @author ronaldo
 */
public class ControleAgendaAtendimentoInternosPedagogia {
    ...
    public AgendamentoAtendimentoInterno incluirAgendaAtendimento(AgendamentoAtendimentoInterno objAgendaAtend) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO AGENDA_ATENDIMENTO_INTERNOS_PEDAGOGIA (StatusReg,DataReg,Departamento,DataAg,Observacao,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public AgendamentoAtendimentoInterno alterarAgendaAtendimento(AgendamentoAtendimentoInterno objAgendaAtend) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AGENDA_ATENDIMENTO_INTERNOS_PEDAGOGIA SET StatusReg=?,DataReg=?,Departamento=?,DataAg=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdReg='" + objAgendaAtend.getIdReg() + "'");
            pst.setString(1, objAgendaAtend.getStatusReg());
            pst.setTimestamp(2, new java.sql.Timestamp(objAgendaAtend.getDataReg().getTime()));
            pst.setString(3, objAgendaAtend.getDepartamentoAgendamento());
            pst.setTimestamp(4, new java.sql.Timestamp(objAgendaAtend.getDataAg().getTime()));
    ...
    public AgendamentoAtendimentoInterno excluirAgendaAtendimento(AgendamentoAtendimentoInterno objAgendaAtendobjAgendaAtend) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM AGENDA_ATENDIMENTO_INTERNOS_PEDAGOGIA WHERE IdReg='" + objAgendaAtendobjAgendaAtend.getIdReg() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
