...
import static gestor.Visao.TelaAberturaRegistroSistema.pADMISSAO_ef;
 ...
 */
public class ListagemAdmissaoEducacaoFisicaM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusEF "
                    + "FROM ADMISSAO_EDUCACAO_FISICA "
                    + "WHERE StatusEF='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataRegistroEF BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
            ...
            while (conecta.rs.next()) {
                ...
                pADMISSAO_ef = pADMISSAO_ef + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemAdmissaoEducacaoFisicaM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
