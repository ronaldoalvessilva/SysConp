 ...
 * @author Ronaldo
 */
public class ControleHistoricoMovimentacaoNUTRI {
    ...
    public HistoricoMovimentacaoEstoque incluirHistoricoProdutoNUTRI(HistoricoMovimentacaoEstoque objHistMovAC) {
        buscarLocalArmazenamento(objHistMovAC.getDescricaoLocal());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_MOVIMENTACAO_ESTOQUE_NUTRI (IdProd,IdLocal,TipoOpe,NomeOperacao,IdDoc,DataMov,QtdItem,SaldoAtual) VALUES (?,?,?,?,?,?,?,?)");
    ...
    public HistoricoMovimentacaoEstoque alterarHistoricoProdutoNUTRI(HistoricoMovimentacaoEstoque objHistMovAC) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE HISTORICO_MOVIMENTACAO_ESTOQUE_NUTRI SET IdProd=?,IdLocal=?,TipoOpe=?,NomeOperacao=?,IdDoc=?,DataMov=?,QtdItem=?,SaldoAtual=? WHERE IdProd='" + objHistMovAC.getIdProd() + "'");
            pst.setInt(1, objHistMovAC.getIdProd());
    ...
    public HistoricoMovimentacaoEstoque excluirHistoricoProdutoNUTRI(HistoricoMovimentacaoEstoque objHistMovAC) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM HISTORICO_MOVIMENTACAO_ESTOQUE_NUTRI WHERE IdProd='" + objHistMovAC.getIdProd() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
