 ...
 * @author ronaldo
 */
public class ControleItensEstornoMateriaisNUTRI {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensRequisicaoMateriaisInternos objItensReqMatInter = new ItensRequisicaoMateriaisInternos();
    int codProd;

    public ItensRequisicaoMateriaisInternos incluirEstornoMaterialInternosNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_ESTORNO_PRODUTOS_NUTRI (IdEst,IdProd,CodigoBarras,UnidadeProd,QtdItem,ValorUnitarioItem,ValorTotalItem,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensRequisicaoMateriaisInternos alterarEstornoMaterialInternosNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_ESTORNO_PRODUTOS_NUTRI SET IdEst=?,IdProd=?,CodigoBarras=?,UnidadeProd=?,QtdItem=?,ValorUnitarioItem=?,ValorTotalItem=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.setInt(1, objItensReqMatInter.getIdReq());
            pst.setInt(2, codProd);
            pst.setString(3, objItensReqMatInter.getCodigoBarras());
            pst.setString(4, objItensReqMatInter.getUnidadeProd());
    ...
    public ItensRequisicaoMateriaisInternos excluirEstornoMaterialInternosNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_ESTORNO_PRODUTOS_NUTRI WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public ItensRequisicaoMateriaisInternos modificarRequisicaoMaterialAvulsoNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_AVULSA_PRODUTOS_NUTRI SET EstornoProduto=? WHERE IdProd='" + objItensReqMatInter.getIdProd() + "'AND IdReq='" + objItensReqMatInter.getIdReq() + "'");
    ...
    public ItensRequisicaoMateriaisInternos modificarRequisicaoMaterialInternosNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_PRODUTOS_INTERNOS_NUTRI SET EstornoProduto=? WHERE IdProd='" + objItensReqMatInter.getIdProd() + "'AND IdReq='" + objItensReqMatInter.getIdReq() + "'");
            pst.setString(1, objItensReqMatInter.getEstornoProduto());
            pst.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Não Foi possivel ALTERAR itens da requisição.\nERRO: " + ex);
