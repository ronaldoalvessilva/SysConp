...
import static gestor.Visao.TelaFechamentoSistema.pSOLICITACAO_far;
 ...
 */
public class ListagemSolicitacaoFARM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusSol "
                    + "FROM SOLICITACAO_PRODUTOS_ADM "
                    + "WHERE StatusSol='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataSol BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusSol"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pSOLICITACAO_far = pSOLICITACAO_far + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemSolicitacaoFARM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
