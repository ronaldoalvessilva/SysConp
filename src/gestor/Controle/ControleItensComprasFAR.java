 ...
 * @author ronaldo
 */
public class ControleItensComprasFAR {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensNfeCompras objItensCompras = new ItensNfeCompras();
    int codProd;

    public ItensNfeCompras incluirItensComprasFAR(ItensNfeCompras objItensCompras) {
        buscarProduto(objItensCompras.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_COMPRAS_MATERIAIS_FAR "
    ...
    public ItensNfeCompras alterarItensComprasFAR(ItensNfeCompras objItensCompras) {
        buscarProduto(objItensCompras.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_COMPRAS_MATERIAIS_FAR SET "
                    + "IdPedido=?,IdNfEntrada=?,IdProd=?,Lote=?,QtdItem=?,ValorUN=?,AliquotaICMS=?,AliquotaIPI=?,DataVctoLote=?, "
    ...
    public ItensNfeCompras excluirItensComprasFAR(ItensNfeCompras objItensCompras) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_COMPRAS_MATERIAIS_FAR WHERE IdItem='" + objItensCompras.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
