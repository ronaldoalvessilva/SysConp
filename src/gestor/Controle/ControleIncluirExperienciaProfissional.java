 ...
 * @author Ronaldo
 */
public class ControleIncluirExperienciaProfissional {
    ...
    public HistoricoEducacionalLaboral incluirExperiencia(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_PROFISSAO_TO_HISTORICO_PROFISSIONAL (IdLanc,IdCodigoProf,IdHistoricoLab,InteresseLabor) VALUES(?,?,?,?)");
            pst.setInt(1, objHistEducLabor.getIdAtend());
            pst.setInt(2, codProf);
            pst.setInt(3, objHistEducLabor.getIdHistoricoLab());
     ...
     public HistoricoEducacionalLaboral excluirExperiencia(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_PROFISSAO_TO_HISTORICO_PROFISSIONAL WHERE IdHistoricoLab='" + objHistEducLabor.getIdHistoricoLab()+ "'");        
            pst.executeUpdate();
        } catch (Exception e) {
