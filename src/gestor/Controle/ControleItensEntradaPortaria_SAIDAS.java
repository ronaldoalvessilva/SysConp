...
public class ControleItensEntradaPortaria_SAIDAS {
    ...
    public ItensEntradaInternosPortaria alterar_ITENTRANSFERENCIA_CANCELADO(ItensEntradaInternosPortaria objItensEntIntPort) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENSTRANSFERENCIA SET SaidaConfirmada=?,RegistroCancelado=? WHERE IdInternoCrc='" + objItensEntIntPort.getIdInternoCrc() + "'AND IdTransf='" + objItensEntIntPort.getIdRetorno() + "'AND IdItem='" + objItensEntIntPort.getIdEntraSaida() + "'");
            pst.setString(1, objItensEntIntPort.getSaidaConfirmada());
            pst.setString(2, objItensEntIntPort.getRegistroCancelado());
            pst.executeUpdate();
        } catch (SQLException ex) {
