 ...
 * @author ronaldo
 */
public class ControleOcorrenciasEducacaoFisica {
    ...
    public OcorrenciasPortaria incluirOcorrenciaP1(OcorrenciasPortaria objOcorr) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO OCORRENCIAS_EF (StatusLanc,DataLanc,Titulo,TextoArea,UsuarioInsert,DataInsert,HorarioInsert,Fonte,Tamanho,BtEsq,BtCen,BtDir,BtJus) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public OcorrenciasPortaria alterarOcorrenciaP1(OcorrenciasPortaria objOcorr) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE OCORRENCIAS_EF SET StatusLanc=?,DataLanc=?,Titulo=?,TextoArea=?,UsuarioUp=?,DataUp=?,HorarioUp=?,Fonte=?,Tamanho=?,BtEsq=?,BtCen=?,BtDir=?,BtJus=? WHERE IdLanc='" + objOcorr.getIdLanc() + "'");
            pst.setString(1, objOcorr.getStatusLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objOcorr.getDataLanc().getTime()));
            pst.setString(3, objOcorr.getTitulo());
            pst.setString(4, objOcorr.getTextoArea());
    ...
    public OcorrenciasPortaria excluirOcorrenciaP1(OcorrenciasPortaria objOcorr) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM OCORRENCIAS_EF WHERE IdLanc='" + objOcorr.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public OcorrenciasPortaria finalizarOcorrenciaP1(OcorrenciasPortaria objOcorr) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE OCORRENCIAS_EF SET StatusLanc=? WHERE IdLanc='" + objOcorr.getIdLanc() + "'");
            pst.setString(1, objOcorr.getStatusLanc());
            pst.executeUpdate();
        } catch (SQLException ex) {
