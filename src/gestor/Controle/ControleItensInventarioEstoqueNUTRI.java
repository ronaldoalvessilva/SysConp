 ...
 * @author user
 */
public class ControleItensInventarioEstoqueNUTRI {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensInventarioEstoque objItensInventEstoque = new ItensInventarioEstoque();
    int codProd;

    public ItensInventarioEstoque incluirItensInventarioNUTRI(ItensInventarioEstoque objItensInventEstoque) {
        buscarProduto(objItensInventEstoque.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_INVENTARIO_NUTRI (IdLanc,IdProd,QtdItem,ValorCusto,IdLocal,Lote,DataLote,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensInventarioEstoque alterarItensInventarioNUTRI(ItensInventarioEstoque objItensInventEstoque) {
        buscarProduto(objItensInventEstoque.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_INVENTARIO_NUTRI SET IdLanc=?,IdProd=?,QtdItem=?,ValorCusto=?,IdLocal=?,Lote=?,DataLote=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensInventEstoque.getIdItem() + "'");
            pst.setInt(1, objItensInventEstoque.getIdLanc());
            pst.setInt(2, codProd);
            pst.setFloat(3, objItensInventEstoque.getQtdItem());
            pst.setFloat(4, objItensInventEstoque.getValorCusto());
    ...
    public ItensInventarioEstoque excluirItensInventarioNUTRI(ItensInventarioEstoque objItensInventEstoque) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_INVENTARIO_NUTRI WHERE IdItem='" + objItensInventEstoque.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
