...
import static gestor.Visao.TelaAberturaRegistroSistema.pCANCELA_visitas;
 ...
 */
public class ListagemCancelamentoVisitasExternaInteraRolM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusCan "
                    + "FROM CANCELAMENTO_VISITAS_EXTERNA_INTERNA_ROL "
                    + "WHERE StatusCan='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataCan BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusCan"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pCANCELA_visitas = pCANCELA_visitas + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemCancelamentoVisitasExternaInteraRolM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
