 ...
 * @author Ronaldo
 */
public class ControleAtualizaDadosUltimaCompraNUTRI {
    ...
    public ProdutoMedicamento alterarDadosComprasNUTRI(ProdutoMedicamento objProdMed) {        
    ...
    public HistoricoProdutoFornecedor incluirDadosFornecedorNUTRI(HistoricoProdutoFornecedor objHistorForn) {
        buscarFornecedor(objHistorForn.getNomeFornecedor());       
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_PRODUTOS_FORNECEDOR_NUTRI (NfCompra,DataMov,IdForn,IdItem,QtdeCompra,ValorUnit) VALUES(?,?,?,?,?,?)");
            ...
            pst.setFloat(5, objHistorForn.getQtdeCompra());
            pst.setFloat(6, objHistorForn.getValorUnit());
            pst.executeUpdate();
        } catch (SQLException ex) {
