 ...
 * @author Ronaldo
 */
public class ControleInstituicaoReligiosa {
    ...
    public Instituicao incluirInstituicao(Instituicao objInst) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO INSTITUICAO_RELIGIOSA (StatusInst,DataCad,NomeInstituicao,Endereco,Cidade,Estado,Telefone,Telefone1,Celular,Celular1,Email,Contato,Observacao,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public Instituicao alterarInstituicao(Instituicao objInst) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE INSTITUICAO_RELIGIOSA SET StatusInst=?,DataCad=?,NomeInstituicao=?,Endereco=?,Cidade=?,Estado=?,Telefone=?,Telefone1=?,Celular=?,Celular1=?,Email=?,Contato=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdCod='" + objInst.getIdCod() + "'");
            pst.setString(1, objInst.getStatusInst());
            pst.setTimestamp(2, new java.sql.Timestamp(objInst.getDataCad().getTime()));
            pst.setString(3, objInst.getNomeInstituicao());
            pst.setString(4, objInst.getEndereco());
    ...
    public Instituicao excluirInstituicao(Instituicao objInst) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM INSTITUICAO_RELIGIOSA WHERE IdCod='" + objInst.getIdCod() + "'");           
            pst.executeUpdate();
        } catch (SQLException ex) {
