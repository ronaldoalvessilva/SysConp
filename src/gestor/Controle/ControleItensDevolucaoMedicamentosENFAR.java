 ...
 * @author Ronaldo
 */
public class ControleItensDevolucaoMedicamentosENFAR {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensRequisicaoMateriaisInternos objItensReqMatInter = new ItensRequisicaoMateriaisInternos();
    int codProd;

    public ItensRequisicaoMateriaisInternos incluirItensDevolucaoENFAR(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_DEVOLUCAO_PRODUTOS_ENFERMARIA_ENFAR (IdDevo,IdProd,CodigoBarras,UnidadeProd,QtdItem,ValorUnitarioItem,ValorTotalItem,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensRequisicaoMateriaisInternos alterarItensDevolucaoENFAR(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_DEVOLUCAO_PRODUTOS_ENFERMARIA_ENFAR SET IdDevo=?,IdProd=?,CodigoBarras=?,UnidadeProd=?,QtdItem=?,ValorUnitarioItem=?,ValorTotalItem=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.setInt(1, objItensReqMatInter.getIdReq());
            pst.setInt(2, codProd);
            pst.setString(3, objItensReqMatInter.getCodigoBarras());
            pst.setString(4, objItensReqMatInter.getUnidadeProd());
    ...
    public ItensRequisicaoMateriaisInternos excluirItensDevolucaoENFAR(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_DEVOLUCAO_PRODUTOS_ENFERMARIA_ENFAR WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    // ALTERAR O SALDO DE PRODUTOS NA TABELA LOTE_PRODUTOS_ENF (ENFERMARIA)
    public ItensRequisicaoMateriaisInternos alterarEstoqueMateraisENFAR(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possível ALTERAR saldo de estoque.\nERRO: " + ex);
        }
        conecta.desconecta();
        return objItensReqMatInter;
    }

    // ALTERAR O SALDO DE PRODUTOS NA TABELA LOTE_PRODUTOS_AC (FARMÁCIA)
    public ItensRequisicaoMateriaisInternos alterarEstoqueMateraisFARMACIA(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possível ALTERAR saldo de estoque da Farmácia.\nERRO: " + ex);
        }
        conecta.desconecta();
        return objItensReqMatInter;
    }
