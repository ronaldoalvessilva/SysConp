...
public class ControleAvaliacaoMedica {
    ...
    public AvaliacaoMedica incluirAvaliacaoMedica(AvaliacaoMedica objAvalia) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO AVALIACAO_MEDICA_PSIQUIATRICA (StatusAval,DataAval,IdInternoCrc,TextoAvaliacao,UsuarioInsert,DataInsert,HorarioInsert) VALUES (?,?,?,?,?,?,?)");
    ...
    public AvaliacaoMedica alterarAvaliacaoMedica(AvaliacaoMedica objAvalia) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AVALIACAO_MEDICA_PSIQUIATRICA SET StatusAval=?,DataAval=?,IdInternoCrc=?,TextoAvaliacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdAvalia='" + objAvalia.getIdAvalia() + "'");
            pst.setString(1, objAvalia.getStatusAval());
            pst.setTimestamp(2, new java.sql.Timestamp(objAvalia.getDataAval().getTime()));
            pst.setInt(3, codInterno);
            pst.setString(4, objAvalia.getTextoAvaliacao());
