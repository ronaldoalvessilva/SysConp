 ...
 * @author Ronaldo
 */
public class ControleItensSolicitacaoMateriaisENFAR {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensSolicitacaoCompras objItensCompra = new ItensSolicitacaoCompras();
    int codProd;

    public ItensSolicitacaoCompras incluirSolicitacaoMaterial(ItensSolicitacaoCompras objItensCompra) {
        buscarProduto(objItensCompra.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_REQUISICAO_PRODUTOS_ENFERMARIA_ENFAR (IdSol,IdProd,CodigoBarras,UnidadeProd,QtdItem,ValorUnitarioItem,ValorTotalItem,ReqAtend,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensSolicitacaoCompras alterarSolicitacaoMaterial(ItensSolicitacaoCompras objItensCompra) {
        buscarProduto(objItensCompra.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_PRODUTOS_ENFERMARIA_ENFAR SET IdSol=?,IdProd=?,CodigoBarras=?,UnidadeProd=?,QtdItem=?,ValorUnitarioItem=?,ValorTotalItem=?,ReqAtend=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensCompra.getIdItem() + "'");
            pst.setInt(1, objItensCompra.getIdSol());
            pst.setInt(2, codProd);
            pst.setString(3, objItensCompra.getCodigoBarras());
            pst.setString(4, objItensCompra.getUnidadeProd());
    ...
    public ItensSolicitacaoCompras excluirSolicitacaoMaterial(ItensSolicitacaoCompras objItensCompra) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_REQUISICAO_PRODUTOS_ENFERMARIA_ENFAR WHERE IdItem='" + objItensCompra.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
