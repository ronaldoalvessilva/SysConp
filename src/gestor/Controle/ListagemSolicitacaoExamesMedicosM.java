...
import static gestor.Visao.TelaAberturaTotalSistema.pSOLICITACAO;
 ...
 */
public class ListagemSolicitacaoExamesMedicosM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusSolExame "
                    + "FROM SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO "
                    + "WHERE StatusSolExame='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataSolExame BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusSolExame"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pSOLICITACAO = pSOLICITACAO + 1;
            }
