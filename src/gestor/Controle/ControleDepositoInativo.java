 ...
 * @author ronaldo
 */
public class ControleDepositoInativo {
    ...
    public DepositoInterno incluirDepositos(DepositoInterno objDeposito) {
        ...
        try {
            ...
            pst.setString(10, objDeposito.getTipoTrans());
            pst.execute();
    ...
    public DepositoInterno alterarDepositos(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE DEPOSITO_INATIVOS SET StatusLanc=?,DataLanc=?,IdInternoCrc=?,ValorDeposito=?,Depositante=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=?,Tipo=? WHERE IdDepIna='" + objDeposito.getIdLanc() + "'");
            pst.setString(1, objDeposito.getStatusLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objDeposito.getDataLanc().getTime()));
            pst.setInt(3, codInt);
            pst.setFloat(4, objDeposito.getValorDeposito());
    ...
    public DepositoInterno alterarDepositosTipo(DepositoInterno objDeposito) {        
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE DEPOSITO_INATIVOS SET Tipo=? WHERE IdDepIna='" + objDeposito.getIdLanc() + "'");
            pst.setString(1, objDeposito.getTipoTrans());           
            pst.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Não Foi possivel ALTERAR o tipo.\n\nERRO: " + ex);
        }
        conecta.desconecta();
        return objDeposito;
    }
    ...
    public DepositoInterno excluirDepositos(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM DEPOSITO_INATIVOS WHERE IdDepIna='" + objDeposito.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public DepositoInterno finalizarDepositos(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE DEPOSITO_INATIVOS SET StatusLanc=? WHERE IdDepIna='" + objDeposito.getIdLanc() + "'");
            pst.setString(1, objDeposito.getStatusLanc());
            pst.executeUpdate();
        } catch (SQLException ex) {
...
//---------------------------- HISTÓRICO DE MOVIMENTAÇÃO DE AUTORIZAÇÃO DE MODIFICAÇÃO DO DEPOSITO INATIVOS
    ...
    public DepositoInterno incluirHitorico(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_LIBERADORES_BANCO_VIRTUAL_INATIVOS (DataLanc,IdDepIna,IdUsuario,Tipo,ValorLiberado,SaldoAtual) VALUES(?,?,?,?,?,?)");
    ...
    public DepositoInterno alterarHitorico(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE HISTORICO_LIBERADORES_BANCO_VIRTUAL_INATIVOS SET DataLanc=?,IdDepIna=?,IdUsuario=?,Tipo=?,ValorLiberado=? WHERE IdLanc='" + objDeposito.getIdLanc() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objDeposito.getDataLancMov().getTime()));
            pst.setInt(2, codUsu);
    ...
    public DepositoInterno excluirHitorico(DepositoInterno objDeposito) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM HISTORICO_LIBERADORES_BANCO_VIRTUAL_INATIVOS WHERE IdLanc='" + objDeposito.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Não Foi possivel EXCLUIR os Dados.\n\nERRO: " + ex);
        }
