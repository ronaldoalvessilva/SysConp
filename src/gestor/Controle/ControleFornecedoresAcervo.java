 ...
 * @author user
 */
public class ControleFornecedoresAcervo {
    ...
    public Fornecedor incluirFornecedor(Fornecedor objForn) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO FORNECEDORES_ACERVO (ClassFor,StatusFor,RazaoSocial,Cnpj,InsEstadual,Telefone,Telefone1,Celular,Email,Fax,Endereco,Compl,Cep,Cidade,Estado,EnderecoCob,ComplCob,CepCob,CidadeCob,EstadoCob) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public Fornecedor alterarFornecedor(Fornecedor objForn) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE FORNECEDORES_ACERVO SET ClassFor=?,StatusFor=?,RazaoSocial=?,Cnpj=?,InsEstadual=?,Telefone=?,Telefone1=?,Celular=?,Email=?,Fax=?,Endereco=?,Compl=?,Cep=?,Cidade=?,Estado=?,EnderecoCob=?,ComplCob=?,CepCob=?,CidadeCob=?,EstadoCob=? WHERE IdForn='" + objForn.getIdForn() + "'");
            pst.setString(1, objForn.getClassFor());
            pst.setString(2, objForn.getStatusFor());
            pst.setString(3, objForn.getRazaoSocial());            
            pst.setString(4, objForn.getCnpj());
