...
public class ControleTransportadora {
    ...
    public Fornecedor incluirTransportador(Fornecedor objForn) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO TRANSPORTADORA (ClassFor,StatusFor,RazaoSocial,Cnpj,InsEstadual,Telefone,Telefone1,Celular,Email,Fax,Endereco,Compl,Cep,Cidade,Estado,EnderecoCob,ComplCob,CepCob,CidadeCob,EstadoCob,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public Fornecedor alterarTtransportador(Fornecedor objForn) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE TRANSPORTADORA SET ClassFor=?,StatusFor=?,RazaoSocial=?,Cnpj=?,InsEstadual=?,Telefone=?,Telefone1=?,Celular=?,Email=?,Fax=?,Endereco=?,Compl=?,Cep=?,Cidade=?,Estado=?,EnderecoCob=?,ComplCob=?,CepCob=?,CidadeCob=?,EstadoCob=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdForn='" + objForn.getIdForn() + "'");
            pst.setString(1, objForn.getClassFor());
            pst.setString(2, objForn.getStatusFor());
            pst.setString(3, objForn.getRazaoSocial());
            pst.setString(4, objForn.getCnpj());
    ...
    public Fornecedor excluirTransportador(Fornecedor objForn) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM TRANSPORTADORA WHERE IdForn='" + objForn.getIdForn() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
