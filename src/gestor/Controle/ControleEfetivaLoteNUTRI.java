 ...
 * @author ronaldo
 */
public class ControleEfetivaLoteNUTRI {
    ...
    public ProdutoMedicamento incluirLoteProdutoNUTRI(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO LOTE_PRODUTOS_AC (IdProd,Lote,Qtd) VALUES (?,?,?)");
    ...
    public ProdutoMedicamento incluirNovoLoteProdutoNUTRI(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO LOTE_PRODUTOS_AC (IdProd,IdLanc,DataVenc,Lote,Qtd,DataEstoque) VALUES (?,?,?,?,?,?)");
            pst.setInt(1, objProdMed.getIdProd());
    ...
    public ProdutoMedicamento alterarLoteProdutoNUTRI(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE LOTE_PRODUTOS_AC SET IdLanc=?,DataVenc=?,Lote=?,Qtd=?,DataEstoque=? WHERE IdProd='" + objProdMed.getIdProd() + "'");
