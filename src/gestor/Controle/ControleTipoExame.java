...
public class ControleTipoExame {
    ...
    public TipoExameMedico incluirTipoExame(TipoExameMedico objTipoEx) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO TIPOS_EXAMES_MEDICO (StatusExame,Classificacao,DescricaoExame,UsuarioInsert,DataInsert,HorarioInsert) VALUES (?,?,?,?,?,?)");
    ...
    public TipoExameMedico alterarTipoExame(TipoExameMedico objTipoEx) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE TIPOS_EXAMES_MEDICO SET StatusExame=?,Classificacao=?,DescricaoExame=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdExame='" + objTipoEx.getIdExame() + "'");
            pst.setString(1, objTipoEx.getStatusExame());
            pst.setString(2, objTipoEx.getClassificacao());
            pst.setString(3, objTipoEx.getDescricaoExame());
    ...
    public TipoExameMedico excluirTipoExame(TipoExameMedico objTipoEx) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM TIPOS_EXAMES_MEDICO WHERE IdExame='" + objTipoEx.getIdExame() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
