 ...
 */
public class ControleIndicadoresAcompanhaEnfermaria {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    IndicadoresAcompanhamento objPerfilInter = new IndicadoresAcompanhamento();
    int codInt;

    public IndicadoresAcompanhamento incluirIndicadorAcompanhamentoEnfermaria(IndicadoresAcompanhamento objPerfilInter) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO "
                    + "INDICADOR_ACOMPANHAMENTO_INTERNO_ENFERMARIA (IdIndAco,IdInternoCrc,DataReg,"
    ...
    public IndicadoresAcompanhamento alterarIndicadorAcompanhamentoEnfermaria(IndicadoresAcompanhamento objPerfilInter) {
        buscarInternoCrc(objPerfilInter.getNomeInternoPerfil(), objPerfilInter.getIdInternoCrc());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE INDICADOR_ACOMPANHAMENTO_INTERNO_ENFERMARIA SET IdIndAco=?,IdInternoCrc=?,DataReg=?,Diabetes=?,DiabControl=?,QtdDiabetes=?,Hipertensao=?,HiperControl=?,QtdHipertensao=?,Escabiose=?,EscabioseCura=?,QtdEscabiose=?,Hanseniase=?,HanseniaseCura=?,QtdHanseniase=?,Sifilis=?,SifilisCura=?,QtdSifilis=?,Tuberculose=?,TuberculoseCura=?,QtdTuberculose=?,Hiv=?,HivControlada=?,QtdHiv=?,HepatiteB=?,HepatiteBCont=?,QtdHepatiteB=?,HepatiteC=?,HepatiteCCont=?,QtdHepatiteC=?,Dst=?,DstCurada=?,QtdDst=?,Vdlr=?,VdlrCurada=?,QtdVdlr=?,Vacina=?,VacinaCura=?,QtdVacina=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdEnf='" + objPerfilInter.getIdEnf() + "'AND IdIndAco='" + objPerfilInter.getIdIndAco() + "'");
            pst.setInt(1, objPerfilInter.getIdIndAco());
            pst.setInt(2, codInt);
            pst.setTimestamp(3, new java.sql.Timestamp(objPerfilInter.getDataReg().getTime()));
            pst.setString(4, objPerfilInter.getDiabetes());
    ...
    public IndicadoresAcompanhamento excluirIndicadorAcompanhamentoEnfermaria(IndicadoresAcompanhamento objPerfilInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM INDICADOR_ACOMPANHAMENTO_INTERNO_ENFERMARIA WHERE IdEnf='" + objPerfilInter.getIdEnf() + "'AND IdIndAco='" + objPerfilInter.getIdIndAco() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
