 ...
 * @author ronaldo
 */
public class ControleSocial2PsicoSocial {
    ...
    public Social2PaiPsicoSocial incluirSocial2PsicoSocial(Social2PaiPsicoSocial objSocial2Pai) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO SOCIAL2_PAI_PSICOSOCIAL (IdPai,IdInternoCrc,NomeFamiliar,Vinculo,Idade,Ocupacao,Endereco,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public Social2PaiPsicoSocial alterarSocial2PsicoSocial(Social2PaiPsicoSocial objSocial2Pai) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE SOCIAL2_PAI_PSICOSOCIAL SET IdPai=?,IdInternoCrc=?,NomeFamiliar=?,Vinculo=?,Idade=?,Ocupacao=?,Endereco=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdFamiliar='" + objSocial2Pai.getIdFamiliar() + "'");
            pst.setInt(1, objSocial2Pai.getIdPai());
            pst.setInt(2, codInt);
            pst.setString(3, objSocial2Pai.getNomeFamiliar());
            pst.setString(4, objSocial2Pai.getVinculo());
    ...
    public Social2PaiPsicoSocial excluirSocial2PsicoSocial(Social2PaiPsicoSocial objSocial2Pai) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM SOCIAL2_PAI_PSICOSOCIAL WHERE IdFamiliar='" + objSocial2Pai.getIdFamiliar() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
