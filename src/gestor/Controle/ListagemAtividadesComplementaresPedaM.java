...
import static gestor.Visao.TelaAberturaTotalSistema.pATIVIDADES_COMPLEMENTARES;
 ...
 */
public class ListagemAtividadesComplementaresPedaM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        pATIVIDADES_COMPLEMENTARES = 0;
        List<FechamentoRegistros> listaTodasNovaEntrada = new ArrayList<FechamentoRegistros>();
        conecta.abrirConexao();
        try {
            conecta.executaSQL("SELECT StatusAC "
                    + "FROM ATIVIDADES_COMPLEMENTARES_PEDAGOGICA "
                    + "WHERE StatusAC='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataAC BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusAC"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pATIVIDADES_COMPLEMENTARES = pATIVIDADES_COMPLEMENTARES + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemAtividadesComplementaresPedaM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
