 ...
 * @author Ronaldo
 */
public class ControleEvolucaoPAI_NOVO {
    ...
    public EvolucaoPAI incluirEvolucaoPAI(EvolucaoPAI objEvolucaoPAI) {
        buscarInternoCrc(objEvolucaoPAI.getNomeInternoCrc(),objEvolucaoPAI.getIdInternoCrc());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO EVOLUCAO_PAI_NOVO (DataEvolucao,IdPai,IdInternoCrc,TextoEvolucao,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?)");
    ...
    public EvolucaoPAI alterarEvolucaoPAI(EvolucaoPAI objEvolucaoPAI) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE EVOLUCAO_PAI_NOVO SET DataEvolucao=?,IdPai=?,IdInternoCrc=?,TextoEvolucao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdEvolucao='" + objEvolucaoPAI.getIdEvolucaoPAI() + "'AND IdPai='" + objEvolucaoPAI.getIdPai() + "'AND IdInternoCrc='" + objEvolucaoPAI.getIdInternoCrc() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objEvolucaoPAI.getDataEvolucaoPAI().getTime()));
            pst.setInt(2, objEvolucaoPAI.getIdPai());
            pst.setInt(3, codInterno);
            pst.setString(4, objEvolucaoPAI.getTextoEvolucaoPAI());
    ...
    public EvolucaoPAI excluirEvolucaoPAI(EvolucaoPAI objEvolucaoPAI) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM EVOLUCAO_PAI_NOVO  WHERE IdEvolucao='" + objEvolucaoPAI.getIdEvolucaoPAI() + "'AND IdPai='" + objEvolucaoPAI.getIdPai() + "'AND IdInternoCrc='" + objEvolucaoPAI.getIdInternoCrc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
