...
import static gestor.Visao.TelaAberturaRegistroSistema.pCOMPOSICAO;
 ...
 */
public class ListagemComposicaoKitHigieneM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusComp "
                    + "FROM COMPOSICAO_PAGAMENTO_KIT_INTERNOS_LOTE "
                    + "WHERE StatusComp='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataComp BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusComp"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pCOMPOSICAO = pCOMPOSICAO + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemComposicaoKitHigieneM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
