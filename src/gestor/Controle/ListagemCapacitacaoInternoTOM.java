...
import static gestor.Visao.TelaAberturaRegistroSistema.pCAPACITACAO_INTERO_to;
 ...
 */
public class ListagemCapacitacaoInternoTOM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusRegistro "
                    + "FROM CAPACITACAO_INTERNO_TO "
                    + "WHERE StatusRegistro='" + pSTATUS_NOVA_ENTRADA + "' "
            ...
            while (conecta.rs.next()) {
                ...
                pCAPACITACAO_INTERO_to = pCAPACITACAO_INTERO_to + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemCapacitacaoInternoTOM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
