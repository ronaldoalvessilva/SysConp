 ...
 * @author user
 */
public class ControleInventarioEstoqueAC {
    ...
    public InventarioEstoque incluirInventarioAC(InventarioEstoque objInventEstoque) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO INVENTARIO_AC (StatusLanc,TipoInventario,IdLocal,Responsavel,DataInicio,DataTermino,HorarioInicio,HorarioTermino,Observacao,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public InventarioEstoque alterarInventarioAC(InventarioEstoque objInventEstoque) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE INVENTARIO_AC SET StatusLanc=?,TipoInventario=?,IdLocal=?,Responsavel=?,DataInicio=?,DataTermino=?,HorarioInicio=?,HorarioTermino=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdLanc='" + objInventEstoque.getIdLanc() + "'");
            pst.setString(1, objInventEstoque.getStatusLanc());
            pst.setString(2, objInventEstoque.getTipoInventario());
            pst.setInt(3, codLocal);
            pst.setString(4, objInventEstoque.getResponsavel());
