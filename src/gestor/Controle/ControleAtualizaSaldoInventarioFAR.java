 ...
 * @author ronaldo
 */
public class ControleAtualizaSaldoInventarioFAR {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ProdutoMedicamento objProdMed = new ProdutoMedicamento();

    public ProdutoMedicamento incluirEstoqueProdutoFAR(ProdutoMedicamento objProdMed) {
    ...
    public ProdutoMedicamento alterarEstoqueProdutoFAR(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
    ...
    public ProdutoMedicamento atualizarEstoqueProdutoFAR(ProdutoMedicamento objProdMed) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE SALDO_ESTOQUE_AC SET SaldoAtual=? WHERE IdProd='" + objProdMed.getIdProd() + "'");
            pst.setFloat(1, objProdMed.getSaldoAtual());
