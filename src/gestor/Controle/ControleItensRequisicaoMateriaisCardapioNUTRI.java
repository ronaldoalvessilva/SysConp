 ...
 * @author Ronaldo
 */
public class ControleItensRequisicaoMateriaisCardapioNUTRI {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensRequisicaoMateriaisInternos objItensReqMatInter = new ItensRequisicaoMateriaisInternos();
    int codProd;

    public ItensRequisicaoMateriaisInternos incluirItensRequisicaoMaterialCardapioNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_REQUISICAO_PRODUTOS_CARDAPIO_NUTRI (IdReq,IdProd,CodigoBarras,UnidadeProd,QtdItem,ValorItem,ValorTotalItem,EstornoProduto,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensRequisicaoMateriaisInternos alterarItensRequisicaoMaterialCardapioNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {
        buscarProduto(objItensReqMatInter.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REQUISICAO_PRODUTOS_CARDAPIO_NUTRI SET IdReq=?,IdProd=?,CodigoBarras=?,UnidadeProd=?,QtdItem=?,ValorItem=?,ValorTotalItem=?,EstornoProduto=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.setInt(1, objItensReqMatInter.getIdReq());
            pst.setInt(2, codProd);
            pst.setString(3, objItensReqMatInter.getCodigoBarras());
            pst.setString(4, objItensReqMatInter.getUnidadeProd());
    ...
    public ItensRequisicaoMateriaisInternos excluirItensRequisicaoMaterialCardapioNUTRI(ItensRequisicaoMateriaisInternos objItensReqMatInter) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_REQUISICAO_PRODUTOS_CARDAPIO_NUTRI WHERE IdItem='" + objItensReqMatInter.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
