...
import static gestor.Visao.TelaAberturaRegistroSistema.pATIVIDADE_UNIDADE;
 ...
 */
public class ListagemAtividadesUnidadesM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        pATIVIDADE_UNIDADE = 0;
        List<FechamentoRegistros> listaTodasNovaEntrada = new ArrayList<FechamentoRegistros>();
        conecta.abrirConexao();
        try {
            conecta.executaSQL("SELECT StatusAtividade "
                    + "FROM ATIVIDADES_UNIDADE "
                    + "WHERE StatusAtividade='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataCriacao BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusAtividade"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pATIVIDADE_UNIDADE = pATIVIDADE_UNIDADE + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemAtividadesUnidadesM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
