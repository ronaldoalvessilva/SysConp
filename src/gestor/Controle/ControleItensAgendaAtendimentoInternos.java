 ...
 * @author ronaldo
 */
public class ControleItensAgendaAtendimentoInternos {
    ...
    public ItensAgendaAtendimentoInternos incluirItensAgendaInterno(ItensAgendaAtendimentoInternos objItens) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_AGENDA_ATENDIMENTO_INTERNOS (IdReg,IdInternoCrc,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?)");
    ...
    public ItensAgendaAtendimentoInternos alterarItensAgendaInterno(ItensAgendaAtendimentoInternos objItens) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_AGENDA_ATENDIMENTO_INTERNOS SET IdReg=?,IdInternoCrc=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdReg='" + objItens.getIdReg() + "'");
            pst.setInt(1, objItens.getIdReg());
            pst.setInt(2, codInterno);
    ...
    public ItensAgendaAtendimentoInternos excluirItensAgendaInterno(ItensAgendaAtendimentoInternos objItens) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_AGENDA_ATENDIMENTO_INTERNOS WHERE IdReg='" + objItens.getIdReg() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
