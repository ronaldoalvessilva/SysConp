...
import static gestor.Visao.TelaAberturaTotalSistema.pFICHA_juridica;
 ...
 */
public class ListagemFichaJuridicaM {
    ...
    public List<FechamentoRegistros> read() throws Exception {
        ...
        try {
            conecta.executaSQL("SELECT StatusFicha "
                    + "FROM FICHA_JURIDICA "
                    + "WHERE StatusFicha='" + pSTATUS_NOVA_ENTRADA + "' "
                    + "AND DataFicha BETWEEN'" + pDATA_PESQUISA_FECHAMENTO + "' "
                    + "AND '" + pDATA_PESQUISA_FECHAMENTO1 + "'");
            while (conecta.rs.next()) {
                FechamentoRegistros pNovaEntrada = new FechamentoRegistros();
                pNovaEntrada.setStatusRegistro(conecta.rs.getString("StatusFicha"));
                listaTodasNovaEntrada.add(pNovaEntrada);
                pFICHA_juridica = pFICHA_juridica + 1;
            }
            return listaTodasNovaEntrada;
        } catch (SQLException ex) {
            Logger.getLogger(ListagemFichaJuridicaM.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            conecta.desconecta();
        }
        return null;
