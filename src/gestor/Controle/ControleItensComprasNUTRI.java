 ...
 * @author ronaldo
 */
public class ControleItensComprasNUTRI {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensNfeCompras objItensCompras = new ItensNfeCompras();
    int codProd;

    public ItensNfeCompras incluirItensComprasNUTRI(ItensNfeCompras objItensCompras) {
        buscarProduto(objItensCompras.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_COMPRAS_MATERIAIS_NUTRI "
    ...
    public ItensNfeCompras alterarItensComprasNUTRI(ItensNfeCompras objItensCompras) {
        buscarProduto(objItensCompras.getDescricaoProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_COMPRAS_MATERIAIS_NUTRI SET "
                    + "IdPedido=?,IdNfEntrada=?,IdProd=?,Lote=?,QtdItem=?,ValorUN=?,AliquotaICMS=?,AliquotaIPI=?,DataVctoLote=?, "
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possivel ALTRAR os Dados\nERRO: " + ex);
        }
        conecta.desconecta();
        return objItensCompras;
    }

    public ItensNfeCompras excluirItensComprasNUTRI(ItensNfeCompras objItensCompras) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_COMPRAS_MATERIAIS_NUTRI WHERE IdItem='" + objItensCompras.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
