 ...
 * @author ronaldo
 */
public class ControleHistoricoAvaliacaoEmprego {
    ...
    public HistoricoAvaliacaoSocial incluirHistoricoAvalivacao(HistoricoAvaliacaoSocial objHistAva) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO HISTORICO_VISITA_EMPREGO_SERVICO_SOCIAL (StatusLanc,DataLanc,IdInternoCrc,Titulo,TextoArea,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public HistoricoAvaliacaoSocial alterarHistoricoAvalivacao(HistoricoAvaliacaoSocial objHistAva) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE HISTORICO_VISITA_EMPREGO_SERVICO_SOCIAL SET StatusLanc=?,DataLanc=?,IdInternoCrc=?,Titulo=?,TextoArea=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdLanc='" + objHistAva.getIdLanc() + "'");
            pst.setString(1, objHistAva.getStatusLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objHistAva.getDataLanc().getTime()));
            pst.setInt(3, codInt);
            pst.setString(4, objHistAva.getTitulo());
    ...
    public HistoricoAvaliacaoSocial excluirHistoricoAvalivacao(HistoricoAvaliacaoSocial objHistAva) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM HISTORICO_VISITA_EMPREGO_SERVICO_SOCIAL WHERE IdLanc='" + objHistAva.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Não Foi possivel EXCLUIR os Dados\n\nERRO" + ex);
        }
    ...
    public HistoricoAvaliacaoSocial finalizarHistoricoAvalivacao(HistoricoAvaliacaoSocial objHistAva) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE HISTORICO_VISITA_EMPREGO_SERVICO_SOCIAL SET StatusLanc=? WHERE IdLanc='" + objHistAva.getIdLanc() + "'");
            pst.setString(1, objHistAva.getStatusLanc());
            pst.executeUpdate();
        } catch (SQLException ex) {
