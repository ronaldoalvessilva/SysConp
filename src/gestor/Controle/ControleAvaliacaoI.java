 ...
 * @author Ronaldo
 */
public class ControleAvaliacaoI {
    ...
    public AvaliacaoI incluirAvaliacaoI(AvaliacaoI objAvaliaI) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO AVALIACAO_I(IdLanc,IdInternoCrc,ConhecoHabilidades,AcreditaRealizacoes,EsperoResultados,AcreditoRealizaTrabalho,AcreditoRealizaLar,"
            ...
            pst.setInt(1, objAvaliaI.getIdLanc());
    ...
    public AvaliacaoI alterarAvaliacaoI(AvaliacaoI objAvaliaI) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AVALIACAO_I SET IdLanc=?,IdInternoCrc=?,ConhecoHabilidades=?,AcreditaRealizacoes=?,EsperoResultados=?,AcreditoRealizaTrabalho=?,AcreditoRealizaLar=?,"
                    + "AcreditoDiverteLazer=?,FacoAtividades=?,TenhoExpectativa=?,TenhoObjetoFuturo=?,IdentificoGostos=?,ParticipoProjetosImport=?,TenhoVariosInteresse=?,CostumoComprometo=?,DeEstudante=?,DeTrabalho=?,DeAmigo=?,DeFamiliar=?,"
                    + "ReconhecoPapeis=?,MantenhoVida=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdAvaliaI='" + objAvaliaI.getIdAvaliaI() + "'AND IdLanc='" + objAvaliaI.getIdLanc() + "'");
            pst.setInt(1, objAvaliaI.getIdLanc());
            pst.setInt(2, objAvaliaI.getIdInternoCrc());
            pst.setString(3, objAvaliaI.getConhecoHabilidades());
            pst.setString(4, objAvaliaI.getAcreditaRealizacoes());
    ...
    public AvaliacaoI excluirAvaliacaoI(AvaliacaoI objAvaliaI) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM AVALIACAO_I WHERE IdAvaliaI='" + objAvaliaI.getIdAvaliaI() + "'AND IdLanc='" + objAvaliaI.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (Exception e) {
