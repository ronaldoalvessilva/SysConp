 ...
 * @author user
 */
public class ControleItensInventarioEstoque {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    ItensInventarioEstoque objItensInventEstoque = new ItensInventarioEstoque();
    int codProd;

    public ItensInventarioEstoque incluirItensInventario(ItensInventarioEstoque objItensInventEstoque) {
        buscarProduto(objItensInventEstoque.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_INVENTARIO_FAR (IdLanc,IdProd,QtdItem,ValorCusto,IdLocal,Lote,DataLote,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?)");
    ...
    public ItensInventarioEstoque alterarItensInventario(ItensInventarioEstoque objItensInventEstoque) {
        buscarProduto(objItensInventEstoque.getNomeProduto());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_INVENTARIO_FAR SET IdLanc=?,IdProd=?,QtdItem=?,ValorCusto=?,IdLocal=?,Lote=?,DataLote=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensInventEstoque.getIdItem() + "'");
            pst.setInt(1, objItensInventEstoque.getIdLanc());
            pst.setInt(2, codProd);
            pst.setFloat(3, objItensInventEstoque.getQtdItem());
            pst.setFloat(4, objItensInventEstoque.getValorCusto());
    ...
    public ItensInventarioEstoque excluirItensInventario(ItensInventarioEstoque objItensInventEstoque) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ITENS_INVENTARIO_FAR WHERE IdItem='" + objItensInventEstoque.getIdItem() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
