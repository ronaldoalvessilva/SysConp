...
public class ControleItensRegistroCanceladoCrc_NOVA_ENTRADA {
    ...
    public ItensRegistroCanceladoCrc incluirRegCanceladoNE(ItensRegistroCanceladoCrc objItensRecCancel) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ITENS_REGISTRO_CANCELADO_NE (IdRegCancel,DataSaida,NomeInternoCrc,NrDocumento,HoraSaida,ConfirmacaoRegistro,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?)");
    ...
    public ItensRegistroCanceladoCrc alterarRegCanceladoNE(ItensRegistroCanceladoCrc objItensRecCancel) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ITENS_REGISTRO_CANCELADO_NE SET IdRegCancel=?,DataSaida=?,NomeInternoCrc=?,NrDocumento=?,HoraSaida=?,ConfirmacaoRegistro=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdItem='" + objItensRecCancel.getIdItem() + "'AND NomeInternoCrc='" + objItensRecCancel.getNomeInternoCrc() + "'");
            pst.setInt(1, objItensRecCancel.getIdLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objItensRecCancel.getDataSaida().getTime()));
            pst.setString(3, objItensRecCancel.getNomeInternoCrc());
            pst.setString(4, objItensRecCancel.getNrDocumento());
