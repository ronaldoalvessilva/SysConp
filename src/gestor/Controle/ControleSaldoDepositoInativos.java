 ...
 * @author Ronaldo
 */
public class ControleSaldoDepositoInativos {
    ...
    public ConsultaSaldoInternos incluirSaldo(ConsultaSaldoInternos objSaldo) {
        ...
        try {
            ...
            pst.setFloat(7, objSaldo.getSaldo());
    ...
    public ConsultaSaldoInternos alterarSaldo(ConsultaSaldoInternos objSaldo) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE SALDO_VALORES_INATIVOS SET DataMov=?,IdInternoCrc=?,IdDoc=?,Historico=?,FavorecidoDepositante=?,TipoMov=?,ValorMov=?,SaldoAtual=? WHERE IdSaldoAtual='" + objSaldo.getIdLanc() + "'AND TipoMov='" + objSaldo.getStatusMov() + "'");
            pst.setTimestamp(1, new java.sql.Timestamp(objSaldo.getDataMov().getTime()));
            pst.setInt(2, objSaldo.getIdInternoCrc());
            pst.setInt(3, objSaldo.getIdLanc());
            pst.setString(4, objSaldo.getHistorico());
