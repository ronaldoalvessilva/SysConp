 ...
 * @author user
 */
public class ControleAdminssaoPsicologia {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    AdmissaoPsicologica objAdmPsi = new AdmissaoPsicologica();
    int codInterno;

    public AdmissaoPsicologica incluirAdmissaoPsi(AdmissaoPsicologica objAdmPsi) {
        buscarInterno(objAdmPsi.getNomeInterno(), objAdmPsi.getIdInternoCrc());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ADMISSAOPSI (StatusLanc,DataLanc,IdInternoCrc,PresoAntes,FamiliaPreso,QuemFamiliaPreso,OndePreso,HistoricoCriminal,"
                    ...
                    + "QualMedicamentoUtiliza,PorqueUsaMedicamento,UsoMedicamentos,RecebeVisitas,Familiares,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public AdmissaoPsicologica alterarAdmissaoPsi(AdmissaoPsicologica objAdmPsi) {
        buscarInterno(objAdmPsi.getNomeInterno(), objAdmPsi.getIdInternoCrc());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ADMISSAOPSI SET StatusLanc=?,DataLanc=?,IdInternoCrc=?,PresoAntes=?,FamiliaPreso=?,QuemFamiliaPreso=?,OndePreso=?,HistoricoCriminal=?,"
                    + "UsaDrogras=?,Alcool=?,Cigarro=?,Maconha=?,Crack=?,Cocaina=?,Cola=?,Outros=?,OutrasDrogas=?,QualIdade=?,PorqueUsaDrogas=?,Drogas=?,TratamentoPSI=?,MedicamentoPSI=?,QualMedicamento=?,AcompanhaPSI=?,TranstornoMental=?,"
                    + "DepartamentoEncaminha=?,DataEncaminhamento=?,HoraAcompanha=?,Encaminhamento=?,TratamentoSaude=?,QualTratamentoSaude=?,OndeFazTratamento=?,TratamentoAnteriores=?,"
                    + "SituacaoTraumatica=?,QualSituacaoTraumatica=?,HouveTentativaSuicidio=?,PorQueSuicidio=?,ComoFoiTentarSuicidio=?,OndeTentouSuicidio=?,TentativaSuicidio=?,"
                    + "QualMedicamentoUtiliza=?,PorqueUsaMedicamento=?,UsoMedicamentos=?,RecebeVisitas=?,Familiares=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdLanc='" + objAdmPsi.getIdLanc() + "'");
            pst.setString(1, objAdmPsi.getStatusLanc());
            pst.setTimestamp(2, new java.sql.Timestamp(objAdmPsi.getDataLanc().getTime()));
            pst.setInt(3, codInterno);
            // HISTORICO CRIMINAL
    ...
    public AdmissaoPsicologica excluirAdmissaoPsi(AdmissaoPsicologica objAdmPsi) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ADMISSAOPSI WHERE IdLanc='" + objAdmPsi.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public AdmissaoPsicologica finalizarAdmissaoPsi(AdmissaoPsicologica objAdmPsi) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE ADMISSAOPSI SET StatusLanc=? WHERE IdLanc='" + objAdmPsi.getIdLanc() + "'");
            pst.setString(1, objAdmPsi.getStatusLanc());
            pst.executeUpdate();
        } catch (SQLException ex) {
