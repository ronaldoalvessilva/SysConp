 ...
 * @author Ronaldo
 */
public class ControleAvaliacaoII {
    ...
    public AvaliacaoII incluirAvaliacaoII(AvaliacaoII objAvaliaII) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO AVALIACAO_II(IdLanc,IdInternoCrc,OrganizoTempo,MantenhoPapeis,SouRotina,ConsigoOutros,TenhoSocial,"
            ...
            pst.setInt(1, objAvaliaII.getIdLanc());
    ...
    public AvaliacaoII alterarAvaliacaoII(AvaliacaoII objAvaliaII) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE AVALIACAO_II SET IdLanc=?,IdInternoCrc=?,OrganizoTempo=?,MantenhoPapeis=?,SouRotina=?,ConsigoOutros=?,TenhoSocial=?,"
                    + "PlanejoAgir=?,ConcentroTrabalho=?,IdentificoProblemas=?,IdentificoSolucaoProblemas=?,QuandoAgir=?,ConsigoHigiene=?,ConsigoCotidianas=?,ConsigoFinancas=?,ConsigoCasa=?,SintoPreciso=?,CostumoFrequentar=?,DataAplicacao=?,"
                    + "ResponsavelAplicacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdAvaliaII='" + objAvaliaII.getIdAvaliaII() + "'AND IdLanc='" + objAvaliaII.getIdLanc() + "'");
            pst.setInt(1, objAvaliaII.getIdLanc());
            pst.setInt(2, objAvaliaII.getIdInternoCrc());
            pst.setString(3, objAvaliaII.getOrganizoTempo());
            pst.setString(4, objAvaliaII.getMantenhoPapeis());
    ...
    public AvaliacaoII excluirAvaliacaoII(AvaliacaoII objAvaliaII) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM AVALIACAO_II WHERE IdAvaliaII='" + objAvaliaII.getIdAvaliaII() + "'AND IdLanc='" + objAvaliaII.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (Exception e) {
