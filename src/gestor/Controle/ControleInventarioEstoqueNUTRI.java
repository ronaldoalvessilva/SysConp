 ...
 * @author user
 */
public class ControleInventarioEstoqueNUTRI {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    InventarioEstoque objInventEstoque = new InventarioEstoque();
    int codLocal;

    public InventarioEstoque incluirInventarioAC(InventarioEstoque objInventEstoque) {
        buscarLocalArmazenamento(objInventEstoque.getNomeLocalArmazenamento());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO INVENTARIO_NUTRI (StatusLanc,TipoInventario,IdLocal,Responsavel,DataInicio,DataTermino,HorarioInicio,HorarioTermino,Observacao,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public InventarioEstoque alterarInventarioAC(InventarioEstoque objInventEstoque) {
        buscarLocalArmazenamento(objInventEstoque.getNomeLocalArmazenamento());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE INVENTARIO_NUTRI SET StatusLanc=?,TipoInventario=?,IdLocal=?,Responsavel=?,DataInicio=?,DataTermino=?,HorarioInicio=?,HorarioTermino=?,Observacao=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdLanc='" + objInventEstoque.getIdLanc() + "'");
            pst.setString(1, objInventEstoque.getStatusLanc());
            pst.setString(2, objInventEstoque.getTipoInventario());
            pst.setInt(3, codLocal);
            pst.setString(4, objInventEstoque.getResponsavel());
    ...
    public InventarioEstoque excluirInventarioAC(InventarioEstoque objInventEstoque) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM INVENTARIO_NUTRI WHERE IdLanc='" + objInventEstoque.getIdLanc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public InventarioEstoque efetivarInventarioAC(InventarioEstoque objInventEstoque) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE INVENTARIO_NUTRI SET StatusLanc=?,DataTermino=?,HorarioTermino=? WHERE IdLanc='" + objInventEstoque.getIdLanc() + "'");
            pst.setString(1, objInventEstoque.getStatusLanc());
