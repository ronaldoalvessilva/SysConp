 ...
 * @author ronaldo
 */
public class ControleParcelasCompras {
    ...
    public ParcelasPagtoCompras incluirParcelasCompras(ParcelasPagtoCompras objParCompras) {
        buscarFornecedor(objParCompras.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO PARCELAS_COMPRAS "
    ...
    public ParcelasPagtoCompras alterarParcelasCompras(ParcelasPagtoCompras objParCompras) {
        buscarFornecedor(objParCompras.getNomeFornecedor());
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE PARCELAS_COMPRAS SET "
                    + "IdNfEntrada=?,IdForn=?,DataVcto=?,ValorParcela=?,ValorTotalParcelas=?, "
    ...
    public ParcelasPagtoCompras excluirParcelasCompras(ParcelasPagtoCompras objParCompras) {

        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM PARCELAS_COMPRAS WHERE IdParc='" + objParCompras.getIdParc() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
