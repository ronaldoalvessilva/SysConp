 ...
 * @author ronaldo
 */
public class ControleGrupoProdutosAC {

    ConexaoBancoDados conecta = new ConexaoBancoDados();
    GrupoProdutos objGrupoProd = new GrupoProdutos();

    public GrupoProdutos incluirGrupo(GrupoProdutos objGrupoProd) {
    ...
    public GrupoProdutos alterarGrupo(GrupoProdutos objGrupoProd) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE GRUPO_PRODUTOS_AC SET StatusGru=?,NomeGrupo=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdGrupo='" + objGrupoProd.getIdGrupo() + "'");
            pst.setString(1, objGrupoProd.getStatusGru());
            pst.setString(2, objGrupoProd.getNomeGrupo());
            pst.setString(3, objGrupoProd.getUsuarioUp());
            pst.setString(4, objGrupoProd.getDataUp());
            pst.setString(5, objGrupoProd.getHorarioUp());
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public GrupoProdutos excluirGrupo(GrupoProdutos objGrupoProd) {
        conecta.abrirConexao();
        try {
