 ...
 * @author Ronaldo
 */
public class ControleRetornoEspontaneo {
    ...
    public RetornoEspontaneo incluirRetonoEspontaneo(RetornoEspontaneo objRetEspontaneo) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO RETORNOESPONTANEO (ObsRetorno,StatusRet,IdOp,DataLancRetorno,UsuarioInsert,DataInsert,HorarioInsert) VALUES(?,?,?,?,?,?,?)");
    ...
    public RetornoEspontaneo alterarRetonoEspontaneo(RetornoEspontaneo objRetEspontaneo) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE RETORNOESPONTANEO SET ObsRetorno=?,StatusRet=?,IdOp=?,DataLancRetorno=?,UsuarioUp=?,DataUp=?,HorarioUp=? WHERE IdRetorno='" + objRetEspontaneo.getIdRetorno() + "'");
            pst.setString(1, objRetEspontaneo.getObsRetorno());
            pst.setString(2, objRetEspontaneo.getStatusRet());
            pst.setInt(3, codOpe);
            pst.setTimestamp(4, new java.sql.Timestamp(objRetEspontaneo.getDataLancRetorno().getTime()));
    ...
    public RetornoEspontaneo excluirRetonoEspontaneo(RetornoEspontaneo objRetEspontaneo) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM RETORNOESPONTANEO WHERE IdRetorno='" + objRetEspontaneo.getIdRetorno() + "'");
            pst.executeUpdate();
        } catch (SQLException ex) {
    ...
    public RetornoEspontaneo finalizarRetonoEspontaneo(RetornoEspontaneo objRetEspontaneo) {
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE RETORNOESPONTANEO SET StatusRet=? WHERE IdRetorno='" + objRetEspontaneo.getIdRetorno() + "'");
