 ...
 * @author Ronaldo
 */
public class ControleHistoricoLaborativoDAO {
    ...
    public HistoricoEducacionalLaboral incluirHistoricoLabor(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        try {
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO TO_HISTORICO_PROFISSIONAL_NOVO (IdATN,IdInternoCrc,TemProfissao,QualProfissao,ExperienciaProfissional,QualExperienciaProfissional,DesejaTrabalharUnid,UsuarioInsert,DataInsert,HorarioInsert,InteresseLabor) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
            ...
            JOptionPane.showMessageDialog(null, "Não Foi possível INSERIR os Dados. (TO_HISTORICO_PROFISSIONAL_NOVO)\nERRO: " + e);
        }
        conecta.desconecta();
    ...
    public HistoricoEducacionalLaboral alterarHistoricoLabor(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("UPDATE TO_HISTORICO_PROFISSIONAL_NOVO SET IdATN=?,IdInternoCrc=?,TemProfissao=?,QualProfissao=?,ExperienciaProfissional=?,QualExperienciaProfissional=?,DesejaTrabalharUnid=?,UsuarioUp=?,DataUp=?,HorarioUp=?,InteresseLabor=? WHERE IdHistoricoLabPN='" + objHistEducLabor.getIdHistoricoLabPN()+ "'");
            pst.setInt(1, objHistEducLabor.getIdATN());
            pst.setInt(2, codInterno);
            pst.setString(3, objHistEducLabor.getTemProfissao());
            pst.setString(4, objHistEducLabor.getQualProfissao());
    ...
    public HistoricoEducacionalLaboral excluirHistoricoLabor(HistoricoEducacionalLaboral objHistEducLabor) {
        ...
        conecta.abrirConexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM TO_HISTORICO_PROFISSIONAL_NOVO WHERE IdHistoricoLabPN='" + objHistEducLabor.getIdHistoricoLabPN()+ "'");
            pst.executeUpdate();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Não Foi possível EXCLUIR os Dados. (TO_HISTORICO_PROFISSIONAL_NOVO)\nERRO: " + e);
        }
        conecta.desconecta();
        return objHistEducLabor;
    }
