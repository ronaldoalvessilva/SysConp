...
-- ASSINATURA BIOMETRICA DA SAIDA LABORATIVA E EDUCACIONAL

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ITENSLABORINTERNO') AND NAME = 'AssinaturaSaida')
   ALTER TABLE ITENSLABORINTERNO ADD AssinaturaSaida VARBINARY(MAX) NULL

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ITENSLABORINTERNO') AND NAME = 'AssinaturaEntrada')
   ALTER TABLE ITENSLABORINTERNO ADD AssinaturaEntrada VARBINARY(MAX) NULL


IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('INTERNOS_ENTRADA_SAIDA_EDUCACAO') AND NAME = 'AssinaturaSaida')
   ALTER TABLE INTERNOS_ENTRADA_SAIDA_EDUCACAO ADD AssinaturaSaida VARBINARY(MAX) NULL

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('INTERNOS_ENTRADA_SAIDA_EDUCACAO') AND NAME = 'AssinaturaEntrada')
   ALTER TABLE INTERNOS_ENTRADA_SAIDA_EDUCACAO ADD AssinaturaEntrada VARBINARY(MAX) NULL

--

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ITENS_SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO') AND NAME = 'ExameReal')
   ALTER TABLE ITENS_SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO ADD ExameReal VARCHAR(30) NULL

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ITENS_SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO') AND NAME = 'MotivoExame')
   ALTER TABLE ITENS_SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO ADD MotivoExame VARCHAR(100) NULL

 
IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO') AND NAME = 'NomeSolicitante')
   ALTER TABLE SOLICITACAO_EXAMES_MEDICO_PSIQUIATRICO ADD NomeSolicitante VARCHAR(200) NULL


IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('HISTORICO_DADOS_PENAIS_INTERNOS'))
	CREATE TABLE HISTORICO_DADOS_PENAIS_INTERNOS
	(
	IdHPenais INT IDENTITY (1,1) PRIMARY KEY NOT NULL,
	IdInternoCrc INT FOREIGN KEY REFERENCES PRONTUARIOSCRC(IdInternoCrc) NOT NULL,
	IdUnid INT FOREIGN KEY REFERENCES UNIDADE(IdUnid) NOT NULL,
	IdEntrada INT FOREIGN KEY REFERENCES ENTRADALOTE(IdEntrada) NOT NULL,
	IdItem INT FOREIGN KEY REFERENCES ITENSENTRADA(IdItem) NOT NULL,
...
IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ODONTOGRAMA_ADMISSAO') AND NAME = 'TipoProc')
   ALTER TABLE ODONTOGRAMA_ADMISSAO ADD TipoProc INT NULL
...
IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('ADVOGADOS') AND NAME = 'ObsAdvogado')   
...
IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('VISITASDIVERSAS') AND NAME = 'ObsVisita')   
...
IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('OFICIAL_JUSTICA') AND NAME = 'ObsOficial')   
...
IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('ELIMIAR_ASSINATURA_INTERNO_PSP'))
	CREATE TABLE ELIMIAR_ASSINATURA_INTERNO_PSP
	(
	IdCancel INT IDENTITY(1,1) PRIMARY KEY NOT NULL,   	
...
-- SCRIPT PARA CRIAR OS NÚMEROS DOS DEPARTAMENTOS.


IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('DEPARTAMENTOS') AND NAME = 'SalaNr')
   ALTER TABLE DEPARTAMENTOS ADD SalaNr INT NULL
