...
IF NOT EXISTS (SELECT NULL FROM SYSOBJECTS WHERE ID = OBJECT_ID('CANCELAMENTO_EVASAO_INTERNOS'))
	...
	-- PARÂMETRO PARA GERAR POPULAÇÃO AUTOMÁTICA OU NÃO
...
UPDATE PARAMETROSCRC SET PopulacaoAutomatica='Não',FecharSistema='Não'
...
-- ALERTA DE SAIDA DE INTERNOS CRC PARA O ALMOXARIFADO

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('PARAMETROSCRC') AND NAME = 'HabilitarAlertaAlmox')
          ALTER TABLE PARAMETROSCRC ADD HabilitarAlertaAlmox VARCHAR(5) NULL 

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('PARAMETROSCRC') AND NAME = 'UsuarioAlmoxarifado01')
          ALTER TABLE PARAMETROSCRC ADD UsuarioAlmoxarifado01 VARCHAR(200) NULL 

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('PARAMETROSCRC') AND NAME = 'UsuarioAlmoxarifado02')
          ALTER TABLE PARAMETROSCRC ADD UsuarioAlmoxarifado02 VARCHAR(200) NULL 

IF NOT EXISTS(SELECT 1 FROM SYSCOLUMNS WHERE ID = OBJECT_ID('PARAMETROSCRC') AND NAME = 'UsuarioAlmoxarifado03')
          ALTER TABLE PARAMETROSCRC ADD UsuarioAlmoxarifado03 VARCHAR(200) NULL 